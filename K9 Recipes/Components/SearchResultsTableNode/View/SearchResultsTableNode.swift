...
class SearchResultsTableNode: TableNodeBase, ASTableDelegate, ASTableDataSource, UITableViewDelegate {
    
    var data: [SearchResultsTableNodeItem]?
    var onTap: (() -> Void)?
    ...
    convenience init(with items: [SearchResultsTableNodeItem]?) {
        self.init()
        self.data = items
    }
    ...
    func tableNode(_ tableNode: ASTableNode, nodeBlockForRowAt indexPath: IndexPath) -> ASCellNodeBlock {
        ...
        return { SearchResultsTableNodeCell(item: itemModel) }
    }
    ...
    func tableNode(_ tableNode: ASTableNode, constrainedSizeForRowAt indexPath: IndexPath) -> ASSizeRange {
        return ASSizeRange(min: CGSize(width: 0, height: 0), max: CGSize(width: 0, height: tableNode.style.preferredSize.width))
    }
